...
public class resController {
    ...
    void reserve(MouseEvent event) throws IOException {
    	if(Bookid.getText().toString().isEmpty()) {
    		...
    		boolean m=s1.ReserveBook(Bookid.getText().toString());
    		if(m) {
    			AlertMsg.mainmsg("Book reserved", "Reservation Successful");
    		}
    		else {
    			AlertMsg.mainmsg("Book already reserved", "Reservation Failed");
    		}
